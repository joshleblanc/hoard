...
module Hoard
  class Tween
    ...
    def internal_update(dt)
      ...
      if @ln < 1
        ...
        p "Calling setter with #{val}, from: #{from}, to: #{to},  #{speed}"
        @setter.call(val)
        @on_update&.call
        @on_update_t&.call(@ln)
      else
  ...
  class Tweenie
    ...
    attr_reader :base_fps, :all_tweens
    ...
    def create(getter, setter, from, to, tp = nil, duration_ms = DEFAULT_DURATION, allow_duplicates = false)
